...
export default function AdminDashboard() {
  ...
  const pending   = employees.filter(e => !e.password).length
  const active    = employees.filter(e => e.password && !e.onboardingComplete).length
  const completed = employees.filter(e => e.onboardingComplete).length
  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        {/* Page title */}
        <div style={{ marginBottom: 32 }} className="animate-fadeUp">
          <h1 style={styles.h1}>Onboarding Dashboard</h1>
          <p style={styles.sub}>Manage and track new employee onboarding.</p>
        </div>

        {/* Stats */}
        <div style={styles.statsRow} className="animate-fadeUp">
          {[
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '32px 0 16px' }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: T.heading }}>People</h2>
